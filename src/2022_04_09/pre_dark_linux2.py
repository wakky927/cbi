...
if __name__ == '__main__':
    ...
    for q_dir in Q_DIR:
        for f in tqdm(range(3800, 5000)):
            ...
            np.savetxt(
                SUPER_DIR2 + "/result/2022_04_09/pre_dark/top/" + q_dir + "/" + sub_dir + f"/{f:06}.csv",
                result_top,
                delimiter=',', fmt="%d", header="(i, j) of upper left coordinates."
            )
            np.savetxt(
                SUPER_DIR2 + "/result/2022_04_09/pre_dark/left/" + q_dir + "/" + sub_dir + f"/{f:06}.csv",
                result_left,
                delimiter=',', fmt="%d", header="(i, j) of upper left coordinates."
            )
            np.savetxt(
                SUPER_DIR2 + "/result/2022_04_09/pre_dark/right/" + q_dir + "/" + sub_dir + f"/{f:06}.csv",
                result_right,
                delimiter=',', fmt="%d", header="(i, j) of upper left coordinates."
            )
